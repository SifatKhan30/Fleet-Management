...
class Maintenance_model extends CI_Model
{
    ...
    {
       $this->db->insert('engine_oil_maintenance', ['vehicle_id'=>$data['vehicle_id'], 'oil_change_date'=>$data['oildate'], 'message'=>$data['message'], 'status'=>$data['status']]);
    }

    public function getreminder()
    {
        $now = date('Y-m-d');
        $lastdate = date('Y-m-d', strtotime($now. ' + 2 days'));

        return $this->db->select('engine_oil_maintenance.*, vehicles.name as vehiclename')
        ->from('engine_oil_maintenance')
        ->join('vehicles', 'vehicles.id=engine_oil_maintenance.vehicle_id')
        ->where("DATEDIFF('".$now."', oil_change_date) >=", 20)
        ->order_by('id','DESC')
        ->get()->result();
    }
